...
object ConvertUtils {
    private fun isSql(text: String?): Boolean {
        ...
        val tempText = fixSql(text).uppercase()
        val select = tempText.contains("SELECT") && tempText.contains("FROM")
        val insert = tempText.contains("INSERT INTO") && tempText.contains("VALUES")
        val delete = tempText.contains("DELETE") && tempText.contains("FROM")
        val update = tempText.contains("UPDATE") && tempText.contains("SET")

        return select || insert || delete || update
    }

    private fun isParam(text: String?): Boolean {
        if (text.isNullOrEmpty()) {
            return false
        }
        ...
        val tempText = fixParam(text)

        val split = tempText.split(",".toRegex()).dropLastWhile { it.isEmpty() }.map { it.trim() }
        for (s in split) {
            if ("null" != s) {
                if (!(s.contains("(") && s.contains(")"))) {
                    return false
                }
                val type = s.substring(s.indexOf("(") + 1, s.indexOf(")"))
    ...
    fun strNum(text: String, checkFlag: String): Int {
        var `var` = text
        var res = 0
    ...
    private fun fixSql(text: String): String {
        val paramFlag = ": "
        return if (text.contains(paramFlag)) {
            text.substring(text.lastIndexOf(paramFlag) + 2).trim()
        } else {
            text.trim()
        }
    }

    private fun fixParam(text: String): String {
        val paramFlag = ": "
        return if (text.contains(paramFlag)) {
            text.substring(text.lastIndexOf(paramFlag) + 2).trim()
        } else {
    ...
    fun convert(sql: String, params: String): String? {
        var resultSql = sql
        if (!isSql(resultSql) || !isParam(params)) {
            return null
        }

        resultSql = fixSql(resultSql)
        if (!resultSql.contains("?")) {
            return null
        }

        val split = fixParam(params).split(",".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
        for (s in split) {
            val trim = s.trim()
            if ("null" == trim) {
                var index = resultSql.indexOf("?")
                val end = index
                var c: Char
                var isNullFlag = 0
                ...
                while (true) {
                    ...
                    if (c != ' ') {
                        if (c == '=') {
                            val preChar = resultSql[index - 1]
                            isNullFlag = when (preChar) {
                                ':', '<', '>' -> 0  // 正常赋值
                                '!' -> 1  // is not null
                                else -> 2  // is null
                            }
                ...
                resultSql = when (isNullFlag) {
                    1 -> resultSql.replaceFirst((resultSql.substring(index, end) + "\\?").toRegex(), " is not null ")
                    2 -> resultSql.replaceFirst((resultSql.substring(index, end) + "\\?").toRegex(), " is null ")
                    0 -> resultSql.replaceFirst("\\?".toRegex(), trim)
                    else -> resultSql.replaceFirst("\\?".toRegex(), trim)
                }
            } else {
                val param = trim.substring(0, trim.indexOf("("))
                val type = trim.substring(trim.indexOf("(") + 1, trim.indexOf(")"))
                val defaultStr = FieldType.getDefaultValue(type)
                val finalParam = defaultStr + param + defaultStr
                resultSql = resultSql.replaceFirst("\\?".toRegex(), finalParam)
            }
